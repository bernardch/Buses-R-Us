...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        ...
        String routeNumber = str.substring(1, str.indexOf("-"));
        String patternName = str.substring(str.indexOf("-")+1, str.indexOf(";"));
        String[] split0 = (str.split("N"+routeNumber+"-"+patternName+";"));
        String split1 = split0[1];
        String[] split2 = split1.split(";");
        int n = 0;
        int max = split2.length;
        List<LatLon> elements = new LinkedList<>();
        while(n < max) {
            String tempLatString = split2[n];
            String tempLonString = split2[n+1];
            Double tempLat = Double.parseDouble(tempLatString);
            Double tempLon = Double.parseDouble(tempLonString);
            LatLon tempLatLon = new LatLon(tempLat, tempLon);
            elements.add(tempLatLon);
            n = n + 2;
        }
        this.storeRouteMap(routeNumber, patternName, elements);
    }
